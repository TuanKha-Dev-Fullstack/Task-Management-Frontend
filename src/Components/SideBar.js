...
function SideBar() {
    ...
    return (
        <div className="w-1/4 h-full p-4 bg-zinc-800 rounded-e-3xl shadow-[10px_0px_20px_rgba(0,0,0,0.5)] fixed bottom-0 flex flex-col">
            ...
            <div className="flex-grow overflow-y-auto custom-scrollbar">
                <ul>
                    {[...Array(20)].map((_, index) => {
                        ...
                        return (
                            <li key={index} className={`hover:bg-blue-300 rounded ${activeLink === link ? 'bg-blue-300' : ''}`}>
                                <Link to={link}
                                    className="font-semibold text-white w-full h-full flex p-2 pl-0"
                                    onClick={() => handleLinkClick(link)}>
                                    <span className={`w-1 mr-4 rounded ${activeLink === link ? 'bg-pink-500' : ''}`}></span>
